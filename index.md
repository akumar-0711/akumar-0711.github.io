...
## Challenges 
There were a good number of challenges that we faced throughout the process of creating this system. The first challenge that we faced was with the dataset itself. The dataset was extremely large, containing 586672 songs with each song having 20 distinct variables. Since there were so many variables, we had difficulty deciding which variables to use and which ones to cut. The main challenge with the unsupervised learning portion of the project was that since our dataset was so large, our Kmeans algorithm took a really long time to run. This limited our progress because it would take hours to check if our code was running properly. The biggest challenge with the supervised learning was choosing which algorithm to use. The first one we tried, linear regression, gave us poor results, however, we were able to achieve our goal using random forest. In performing out dimensionally reduction we found that 9 variables were the most efficient and effective variables to use. However in running a feature importance test, we found that the importance was very skewed towards one variable. In trying to normalize this one variable, it took us hours just due to the sheer size of the dataset. After completing this normalization of the necessary variables, the variable importance became much more balanced with no one variable holding the majority of the decision power. 
