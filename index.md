...
## Challenges 
There were a good number of challenges that we faced throughout the process of creating this system. The dataset to begin with was extremely large, containing thousands of songs along with multiple variables for each of these songs. As the dataset itself included so many songs with so many variables, the unsupervised learning part including K-Means took a very long time to process through. The supervised learning method of linear regression also offered very low accuracy, and we had to find other supervised learning methods to get around this. From our dimensionality reduction we discovered that our optimal number of features was 9, however trying to normalize and makeup for this skew of one variable took us hours to normalize one variable. When determining out feature importance, our results were heavily skewed towards one variable, and did not provide us the output that we exactly wanted. 
